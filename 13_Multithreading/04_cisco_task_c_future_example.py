...
def cisco_cmd_executor(hostname, commands, username, password):
    try:
        ...
        return f"Script execution completed on the device {hostname}"
...
hosts = [vIOS1, vIOS2, vIOS3]

with cf.ThreadPoolExecutor() as executor:
    total_output = [executor.submit(cisco_cmd_executor, **host) for host in hosts]
